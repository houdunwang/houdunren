      ...
      <div class="card-body">
        <div class="form-group">
          ...
          <select class="form-control" id="exampleFormControlSelect1" v-model="rule.wechat_id" size="3">
            <option v-for="(wechat, index) in wechats" :key="index" :value="wechat.id">{{ wechat.title }}</option>
          </select>
        </div>

        <div class="form-group">
          <label>规则名称</label>
          <input type="text" class="form-control" placeholder v-model="rule.title" :required="required" />
        </div>

        <div>
          <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
              <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">普通关键词</a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false">正则关键词</a>
            </li>
          </ul>
          <div class="tab-content border p-3 border-top-0" id="myTabContent">
            <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
              <div class="input-group mb-3" v-for="(keyword, index) in keywords" :key="index" v-show="!keyword.regexp">
                <input type="text" class="form-control" :class="{ 'border-danger': keyword.has }" required v-model="keywords[index].word" @blur="checkKeyword(keywords[index])" />
            ...
            <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
              <div class="input-group mb-3" v-for="(keyword, index) in keywords" :key="index" v-show="keyword.regexp">
                <input type="text" class="form-control" required v-model="keywords[index].word" @blur="checkKeyword(keywords[index])" />
                <a href="#" class="input-group-append" @click.prevent="del(index)">
                  <span class="input-group-text">删除</span>
                </a>
              </div>
