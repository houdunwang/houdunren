<div class="form-group {{ $attributes['class']??'' }}">
  ...
  <div class="input-group  @error($attributes['name'])is-invalid @enderror">
    ...
    <div class="custom-control custom-radio custom-control-inline">
      <input type="radio" value="{{ $value }}" id="{{ $attributes['name'].$value }}" name="{{ $attributes['name'] }}"
        class="custom-control-input" {{ isset($attributes['value']) && $value==$attributes['value']?"checked":'' }}>
