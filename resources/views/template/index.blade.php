    ...
    <section class="border-bottom row align-items-center pb-3 pt-3">
      ...
      <div class="col-sm-1 text-left text-sm-right ">
        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
          ...
          <btn-del action="{{ route('admin.template.uninstall',$template['name']) }}" class="btn btn-secondary">
            删除
          </btn-del>
          @else
          <a href="{{ route('admin.template.install',$template['name']) }}" class="btn btn-outline-success">
            安装
          </a>
          @endif
        </div>
