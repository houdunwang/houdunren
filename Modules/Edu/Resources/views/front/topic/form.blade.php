  ...
  <div class="card shadow-sm">
    ...
    <div class="card-body">
      ...
      <div class="mb-2">
        @foreach ($tags as $tag)
        <label class="mr-3">
          <input type="checkbox" name="tags[]" value="{{ $tag['id'] }}" {{ $topic->tags->contains($tag)?'checked':'' }}>
          {{ strtoupper($tag['title']) }}
        </label>
        @endforeach
      </div>
      <div class="v-cloak">
        <toast-editor :content.sync="content" action="{{ route('common.upload.make') }}" name="content">
        </toast-editor>
