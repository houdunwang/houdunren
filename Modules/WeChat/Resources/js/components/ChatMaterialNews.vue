    ...
    <div class="d-flex mt-3" v-if="wechat_id">
      ...
      <div class="border ml-3 flex-fill p-3 d-flex flex-column" v-if="current">
        <div class="flex-fill">
          ...
          <hd-image-upload :value.sync="current.picurl" />
          <hd-wang-editor :value.sync="current.content" :uploadImgServer="'/common/upload/wangEditorMaterialNewsUpload/' + wechat_id"></hd-wang-editor>
        </div>
...
export default {
  ...
  methods: {
    async getArticle() {
      if (this.id) {
        const response = await this.axios.get(`/WeChat/admin/material/${this.id}`)
    ...
    async submit() {
      ...
      if (this.id) {
        const response = await this.axios.put(`/WeChat/admin/material/${this.id}?type=news`, { wechat_id: this.wechat_id, content: this.news })
      } else {
        const response = await this.axios.post(`/WeChat/admin/material?type=news`, { wechat_id: this.wechat_id, content: this.news })
      }
      location.href = '/WeChat/admin/material?type=news'
    },
  },
}
</script>
