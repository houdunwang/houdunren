...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.id) {
        const response = await this.axios.put(`/WeChat/admin/material/${this.id}?type=voice`, this.$data)
      } else {
        const response = await this.axios.post(`/WeChat/admin/material?type=voice`, this.$data)
      }
      location.href = '/WeChat/admin/material?type=voice'
    },
  },
}
</script>
