...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.id) {
        const response = await this.axios.put(`/WeChat/admin/material/${this.id}?type=image`, this.$data)
      } else {
        const response = await this.axios.post(`/WeChat/admin/material?type=image`, this.$data)
      }
      location.href = '/WeChat/admin/material?type=image'
    },
  },
}
</script>
