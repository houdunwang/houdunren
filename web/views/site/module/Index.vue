...
export default {
  ...
  async created() {
    ...
    this.$set(this, 'site', response[0].data)
    this.$set(this, 'modules', response[1].data)
    this.loadMessage = response[1].data.length ? '' : '暂无可用模块'
  },
  computed: {
  ...
  methods: {
    ...
    async setDefaultModule(mid) {
      await this.axios.get(`site/${this.$route.params.sid}/module/${mid}`)
