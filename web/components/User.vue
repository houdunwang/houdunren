<template>
  <div>
    <div class="mb-3 mb-2">
      <a-input-search placeholder="请输入用户名、手机号或用户编号进行搜索" v-model="key" @search="load" enterButton />
    </div>
    <a-table
      class="border"
      v-if="users.data"
...
export default {
  ...
  methods: {
    async load({ current = 1 }) {
      let response = await this.axios.get(`${this.action}?page=${current}&key=${this.key}`)
