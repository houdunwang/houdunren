        ...
        <el-row :gutter="20">
          <el-col :xs="24" :md="12" v-if="setting.base">
            ...
            <el-form-item label="后台页脚">
              <el-input type="textarea" :rows="4" v-model="setting.base.footer.value" placeholder="在后面底部显示的内容"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-tab-pane>
...
export default {
  ...
  async created() {
    let response = await Promise.all([this.axios.get('system/group'), this.axios.get('/system/config/1')])
